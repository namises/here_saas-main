...
async function handler(req, res) {
  ...
  try {
    ...
    if (`${leaveRequest.owner}` !== employee) throw new Error("You cannot approve this leave");
    ...
    await DB.LeaveRequest.updateOne({ _id: leaveId }, { $set: { status, owner: manager } }, { session });
