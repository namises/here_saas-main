...
const useCreateLeavePolicy = (
  ...
  const loading = useSelector((s) => s.apiStatus[endpoints.leave.policy.create]);
  const [error, setError] = useState({});
  const handleCreateLeavePolicy = async () => {
    const { success, result } = validator(leaveType, code, description, maxPerYear, accrualFrequency, applicableAfterMonths, carryForward, maxCarryForward, encashable, maxEncashable);
    if (success) {
      const res = await API.leave.policy.create(result);
      if (res.success) {
        onCreate();
      }
    } else {
  ...
  useEffect(() => {
    setError({});
  }, [leaveType, code, description, maxPerYear, accrualFrequency, applicableAfterMonths, carryForward, maxCarryForward, encashable, maxEncashable]);

  return { handleCreateLeavePolicy, error, loading };
};

const validator = (leaveType, code, description, maxPerYear, accrualFrequency, applicableAfterMonths, carryForward, maxCarryForward, encashable, maxEncashable) => {
  console.log({ leaveType, code, description, maxPerYear, accrualFrequency, applicableAfterMonths, carryForward, maxCarryForward, encashable, maxEncashable });
  const isInvalid = {};
  const body = {};
...
export default useCreateLeavePolicy;
