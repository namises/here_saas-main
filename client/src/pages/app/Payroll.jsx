...
import { MdFilterAlt, MdPlayCircle } from "react-icons/md";
...
import GeneratePayrollForm from "src/components/GeneratePayrollForm";
  ...
  return (
    <>
      ...
      <div className="flex flex-row justify-end items-center gap-5 mb-5 text-2xl text-white">
        <SolidButton Icon={MdPlayCircle} title="Generate Payroll" onClick={() => setIsGenerateOpen(true)} />
        <SolidButton Icon={MdFilterAlt} title="Filter" onClick={() => setShowFilter(!showFilter)} />
      </div>
      <GeneratePayrollForm isOpen={isGenerateOpen} setIsOpen={setIsGenerateOpen} onGenerated={refresh} />
